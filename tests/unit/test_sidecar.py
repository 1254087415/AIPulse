...
async def test_get_settings_masks_secrets(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # 在 os.environ 里放的 LLM_API_KEY 占位符，让本测试看到的是真正的空 secret。
    ...
    assert result.error is None
    assert result.result["llm_base_url"] == "https://api.minimaxi.com/v1"
    assert result.result["llm_model"] == "MiniMax-M2.5"
...
async def test_main_parses_line_and_responds(monkeypatch, tmp_path: Path) -> None:
    ...
    assert response["result"]["llm_model"] == "MiniMax-M2.5"
