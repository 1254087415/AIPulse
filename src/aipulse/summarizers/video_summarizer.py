"""Video summarizer implementation."""
...
DEFAULT_MAX_TRANSCRIPT_TOKENS = 16000


class VideoSummarizer(Summarizer):
    """Summarize video transcripts using an LLM."""
    ...
    async def summarize(
        ...
        """Summarize a video transcript into a structured result."""
        safe_title = title or "未知视频"
        system_prompt = (
            "你是一个中文视频内容总结助手。请用 JSON 格式输出，不要加 markdown 代码块，直接输出 JSON：\n"
            '{"title": "...", "summary": "...", "key_points": ["..."], "tags": ["..."]}\n'
            "要求：\n"
            "- title 用一句话概括视频主题\n"
            "- summary 用 2-4 段中文总结核心内容\n"
    ...
    def _build_prompt(self, title: str, transcript: str) -> str:
        trimmed = transcript[:DEFAULT_MAX_TRANSCRIPT_TOKENS]
        return f"标题：{title}\n\n字幕：\n{trimmed}"
